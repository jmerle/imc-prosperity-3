...
class JamsStrategy(SignalStrategy):
    def get_signal(self, state: TradingState) -> Signal | None:
        if any(
            v not in state.order_depths for v in ["CROISSANTS", "JAMS", "DJEMBES", "PICNIC_BASKET1", "PICNIC_BASKET2"]
        ):
            return None
...
class PicnicBasket1Strategy(SignalStrategy):
    def get_signal(self, state: TradingState) -> Signal | None:
        if any(v not in state.order_depths for v in ["CROISSANTS", "JAMS", "DJEMBES", "PICNIC_BASKET1"]):
            return None
...
class PicnicBasket2Strategy(SignalStrategy):
    def get_signal(self, state: TradingState) -> Signal | None:
        if any(v not in state.order_depths for v in ["CROISSANTS", "JAMS", "PICNIC_BASKET2"]):
            return None
